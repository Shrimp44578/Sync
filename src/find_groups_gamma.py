...
FILE = "projectdata_2o*e10l2n1.txt"
FILE_PATH = gl.glob(os.path.join(DIRECTORY, FILE))

# Initialize variables
NUM_FILES = 0
results_df = pd.DataFrame(columns=["Gamma", "NUM_GROUPS"])
...
for file in FILE_PATH:
    ...
    results_df.loc[len(results_df)] = {"Gamma": gamma, "NUM_GROUPS": NUM_GROUPS}
    NUM_FILES += 1
...
sns.lineplot(data=results_df, x="Gamma", y="NUM_GROUPS", marker="o", color="b")

# Add titles and labels
plt.title("Number of Groups vs Gamma", fontsize=16)
plt.xlabel("Gamma", fontsize=16)
plt.ylabel("Number of Groups", fontsize=16)

# Set axes to start at 0
# Extend x-axis slightly beyond max Gamma
plt.xlim(left=0, right=results_df["Gamma"].max() + 0.1)
...
plt.xticks(np.arange(0, results_df["Gamma"].max() + 0.2, 0.2))
